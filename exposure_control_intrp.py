...
def test_step(frame, num_frames, model, device, dataset, batch, loss_fn, lpips_loss_fn_alex, lpips_loss_fn_vgg, args, test_losses, test_psnrs, test_ssims, test_lpips_alexs, test_lpips_vggs, resume_step, resample, shading_codes, seed, ids, i):
    ...
    if args.test.save_fig:
        ...
        log_dir = os.path.join(args.save_dir, args.index, 'test', f'exposure_control_intrp_resample{resample}_seed{seed}_ids{ids[0]}-{ids[1]}')
    ...
    for frame, batch in enumerate(testloader):
        ...
        for i in range(num_interpolations):
            ...
            plots = test_step(frame, len(testloader), model, device, dataset, batch, loss_fn, lpips_loss_fn_alex,
                            ...
                            test_lpips_vggs, resume_step, resample, shading_codes, seed, ids, i)
