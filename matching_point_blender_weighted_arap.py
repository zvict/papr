...
def positional_encoding(x, num_frequencies=1):
    pe = []
    for i in range(num_frequencies):
        for fn in [torch.sin, torch.cos]:
            pe.append(fn(x))
...
class ARAPLoss(nn.Module):
    def __init__(self, base_pc, smooth_knn=10, segmentation_masks=None):
        super().__init__()
        ...
        if segmentation_masks is not None:
            if len(segmentation_masks) != self.num_pts:
                raise ValueError("segmentation_masks must have the same length as base_pc.")
            self.segmentation_masks = segmentation_masks.to(self.device)
        else:
            self.segmentation_masks = None
            
        self.neighbor_weights = torch.ones(self.num_pts, smooth_knn, device=self.device) # Default to 1.0
    ...
    def forward(self, pc_transformed, add_lda=False, lda_weight=1.0):
        ...
        weighted_squared_diff = (total_displacement_after_update.pow(2).sum(dim=2)
                                 - self.nn_distances[:, :self.smooth_knn]) * self.neighbor_weights
...
def modified_icp_nn_new(
    ...
    if save_logs:
        save_path = os.path.join(
            log_dir, f"weighted_arap_k{smooth_knn}_iter{iterations}/"
        )
        if len(loss_modes):
    ...
    semantic_parts = []
    for i, tensor_part in enumerate(src):
    ...
    arap_loss = ARAPLoss(torch.cat(src, dim=0), smooth_knn=smooth_knn, segmentation_masks=semantics) 
    # if early_stop:
...
def modified_icp_one(
    ...
    if save_logs:
        save_path = os.path.join(
            log_dir,
            f"temp_{model.temperature}_freq_{model.max_freq}_w_arap_k{smooth_knn}_w{arap_w}_iter{iterations}/",
        )
        if len(loss_modes):
            save_path = save_path[:-1] + "_"
            for lm in loss_modes:
    ...
    arap_loss = ARAPLoss(
        torch.cat(src, dim=0), smooth_knn=smooth_knn, segmentation_masks=semantics
    )
    # if early_stop:
    #     init_volumes = [estimate_volume(src[i].cpu().numpy()) for i in range(len(src))] if len(base_volumes) == 0 else base_volumes
    ...
    for it in tqdm.tqdm(range(iterations), desc="ICP Iterations"):
        ...
        for i in range(len(src)):
            ...
            # Convert axis-angle to rotation matrix
            R = axis_angle_to_rotation_matrix(axis_angle)
            rotated = torch.bmm(R, src[i].unsqueeze(-1)).squeeze(-1)
            cur_transformed = rotated + translation
            src_transformed.append(cur_transformed)
            ...
            cur_ratio = (
                estimate_volume(src_trans_np) / init_volumes[i]
                if len(init_volumes)
                else 1.0
            )
            if (
                len(init_volumes)
                and (cur_ratio < stop_volume_ratio[i] - stop_volume_error_rates[i])
                or (cur_ratio > stop_volume_ratio[i] + stop_volume_error_rates[i])
            ):
                if new_tgts[i] is None:
                    print(f"!! Changing target for part {i}")
                    new_tgts[i] = src_transformed[i].clone().detach()
        ...
        if use_arap:
            loss = loss * 1.0 / total_num_pts + total_arap_loss * arap_w
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()
        ...
        if save_logs:
            tqdm.tqdm.write(
                f"Iteration {it} Loss ({loss_mode}): {loss.item():.6f}, ARAP Loss: {total_arap_loss:.6f}"
            )
        ...
        if save_logs:
            ...
            if it % 10 == 0 or it == iterations - 1 or (stop and early_stop):
                plot_pointcloud(
                    src_transformed,
                    save_path,
                    title=f"Transformed Point Cloud Iter {it}",
                    scale=VISIUAL_SCALE,
                    # extra_points=tgt, plot_match=False
                )
                if plot_match:
                    ...
                    if it % 100 == 0 or (stop and early_stop):
                        imageio.mimsave(
                            os.path.join(save_path, "match_frames.mp4"),
                            match_frames,
                            fps=10,
                        )
                        # torch.save(model.state_dict(), os.path.join(save_path, f"deform_model_{it}.pth"))
                        ...
                        torch.save(
                            model.state_dict(),
                            os.path.join(save_path, f"icp_model_{it}.pth"),
                        )
    ...
    if plot_match and save_logs:
        imageio.mimsave(
            os.path.join(save_path, "match_frames.mp4"), match_frames, fps=10
        )
        # save model weights
        # torch.save(model.state_dict(), os.path.join(save_path, f"deform_model_final.pth"))
        # torch.save(transformations, os.path.join(save_path, f"transformation_final.pth"))
    ...
    return model, src_transformed.detach(), cur_frame, it, all_tgt_indices
...
# scale = 10.0
# src_pc = src_pc / scale
tgt_pc = np.load("/NAS/spa176/papr-retarget/bird_vertices.npy")
...
if USE_ICP:
    converged, rmse, Xt, RTs, t_history = icp(
        tgt_pc.unsqueeze(0), src_pc.unsqueeze(0), max_iterations=300
    )
    print(
...
for part_idx in range(total_part_num):
    for time_step in range(len(src_keypoints[part_idx])):
        ...
        # print(f"Part {part_idx}, Time step {time_step}, src_kp shape: {cur_src_kp.shape}")
...
if TRAIN_ICP_MODEL:
    ...
    for time_step in tqdm.tqdm(range(total_time_steps)):
        ...
        (
            ...
            early_stop=True,
            stop_volume_ratio=[
                src_volume_ratio[time_step] for src_volume_ratio in src_volume_ratios
            ],
            init_volumes=base_volumes,
            stop_volume_error_rates=[0.05, 10.0, 10.0],
            # stop_volume_ratio=0.97,
...
if USE_KEYPOINTS:
    ...
    for cur_step in tqdm.tqdm(range(total_time_steps)):
        ...
        for part_idx in range(total_part_num):
            ...
            deformed_full_point_cloud = rbf(tgt_pcs[part_idx].cpu().numpy())
