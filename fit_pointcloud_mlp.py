...
if TRAIN_DEFORM_NET:
    if USE_POINTNET:
        # add pointnet feature extraction
        # pointnet = pointnet2(50, normal_channel=False).to(device)
        # checkpoint = torch.load("pointnet_seg_model.pth")
        pointnet = pointnet(normal_channel=False).to(device)
        checkpoint = torch.load("best_model.pth")
        pointnet.load_state_dict(checkpoint["model_state_dict"])
        pointnet.eval()
        with torch.no_grad():
            # l1_points, l2_points, l3_points = pointnet(pc_normalize(src_pc).unsqueeze(0).transpose(2, 1))
            # print("l1_points: ", l1_points.shape)
            # print("l2_points: ", l2_points.shape)
            # print("l3_points: ", l3_points.shape)
            point_feat = pointnet(pc_normalize(src_pc).unsqueeze(0).transpose(2, 1)).transpose(2, 1)[0]
            # print("!!out: ", out.transpose(2, 1).shape)
            # print(a)
    ...
    for i in tqdm.tqdm(range(n_iter)):
        ...
        if i % 100 == 0:
            ...
            pc_img = plot_pointcloud(deformed_src_pc, log_dir, title=f"Deformed Point Cloud Iter {i}")
            pc_images.append(pc_img)

    torch.save(net.state_dict(), os.path.join(log_dir, "deform_net.pth"))
    imageio.mimsave(os.path.join(log_dir, 'train_deformed_pc.mp4'), pc_images, fps=10)
...
if REG_DEFORM_NET:
    ...
    if concat_feature:
        if use_keypoints:
            cur_log_dir = os.path.join(
                log_dir,
                f"transform_kp_num_{num_keypoints}_kpnn_{kp_knn}_cdw{cd_loss_w}_rigidw{rigid_loss_w}_ldasw{ldas_loss_w}_nn{num_nn}_concat_{input_case}",
                # f"test_deformed_pc",
            )
        else:
            cur_log_dir = os.path.join(
                log_dir,
                f"transform_L{transform_L}_cdw{cd_loss_w}_rigidw{rigid_loss_w}_ldasw{ldas_loss_w}_nn{num_nn}_concat",
                # f"test_deformed_pc",
            )
    elif no_scale:
        cur_log_dir = os.path.join(
    ...
    for idx in tqdm.tqdm(range(start, end, interval)):
        ...
        if use_keypoints and idx == start:
            # apply farthest point sampling to get the keypoints
            init_kp, kp_indices = sfp(src_pc.unsqueeze(0), K=num_keypoints)

        src_pcs.append(src_pc)
        key_points.append(src_pc[kp_indices[0]])

    src_pcs = torch.stack(src_pcs, dim=0)
    key_points = torch.stack(key_points, dim=0)
    print("src_pcs: ", src_pcs.shape)
    print("key_points: ", key_points.shape)
    ...
    with torch.no_grad():
        if USE_POINTNET:
            ...
            pointnet.load_state_dict(checkpoint["model_state_dict"])
            pointnet.eval()
            point_feat = pointnet(pc_normalize(src_pcs[0].to(device)).unsqueeze(0).transpose(2, 1)).transpose(2, 1)[0]
    ...
    if use_keypoints:
        ...
        pred_src_pcs, src_pc_weights, src_nn_inds = parametertize_pc(src_pc, init_kp[0], kp_knn, step=12000)
        # parameterize the deformed point cloud
        pred_deformed_pc, deformed_pc_weights, deformed_nn_inds = parametertize_pc(
            deformed_src_pc, deformed_src_pc[kp_indices[0]], kp_knn, step=35000
        )
        ...
        normal_rotation_matrices, pred_normals = compute_rotation_matrices_for_batch(
            key_points, 10, flip_normal=flip_normal
        )
        ori_kp_deforms = torch.matmul(
            normal_rotation_matrices,
            init_displacement[:, kp_indices[0], :]
            .detach()
            .cpu()
        ...
        # for visualization
        vis_ori_kp_deforms = ori_kp_deforms[sample_steps]
    ...
    if use_keypoints:
        base_dist_pcs = pred_deformed_pc
    ...
    for pt_idx in range(num_pts):
        ...
        dist_to_all_pts = (
            (base_dist_pcs[pt_idx : pt_idx + 1, :].expand(num_pts, 3) - base_dist_pcs)
            .pow(2)
            .sum(dim=1)
        )
        ...
        dist_to_nn[pt_idx, :], nn_indices[pt_idx, :] = vals[1:], inds[1:]
        gaussian_weight = 0
        # nn_weights[pt_idx, :] = (
    ...
    nn_indices = nn_indices.type(torch.int64)
    # dist_to_nn = (dist_to_nn * nn_weights).detach()
    dist_to_nn = dist_to_nn.detach()
    # nn_init_positions = deformed_src_pc.detach().clone().to(device)
    nn_init_positions = base_dist_pcs.to(device)
    ...
    for i in tqdm.tqdm(range(n_iter)):
        ...
        for j in range(1, num_steps):
            ...
            if concat_feature:
                if input_case == 0:
                    ...
                    scale, quaternion = transform_net(torch.cat([src_pcs[0].to(device), init_displacement[0]], dim=-1))
            else:
            ...
            if use_keypoints:
                final_deformed_src_pcs = torch.einsum(
                    "ij,ijk->ik",
                    deformed_pc_weights,
                    deformed_src_pcs[deformed_nn_inds],
                )
            else:
            ...
            total_cd_loss = chamfer_distance(
                final_deformed_src_pcs.unsqueeze(0), tgt_pc.unsqueeze(0)
            )[0]
            ...
            if concat_feature:
                if input_case == 0:
                    ...
                    scale, quaternion = transform_net(
                        torch.cat([src_pcs[j].to(device), init_displacement[0]], dim=-1)
                    )
            else:
            ...
            if use_keypoints:
                displacement = base_displacement[j]
            else:
            ...
            total_dist_to_nn_after_update = (
                (
                    cur_deformed_src_pcs.view(num_pts, 1, 3).expand(num_pts, num_nn, 3)
                    - cur_deformed_src_pcs.view(num_pts, 1, 3)
            ...
            if loss_type == "L1":
                reg_loss = (
                    (
                        total_dist_to_nn_after_update.sum(dim=1)
                        - dist_to_nn.to(device)
                    )
                    .abs()
                    .sum()
                    / (num_pts * num_nn)
                ...
                reg_loss = torch.nn.functional.mse_loss(
                    total_dist_to_nn_after_update, dist_to_nn[:, :num_nn].to(device)
                )
            ...
            # #         .expand(num_pts, num_nn, 3),
            # #     )
            # #     - nn_init_positions.view(num_pts, 1, 3)
            # #     .expand(num_pts, num_pts, 3)
            # #     .gather(
            # #         0,
            # #         nn_indices[:, :num_nn]
            ...
            # #     (deformed_src_pcs[j] - (avg_displacement + nn_init_positions))
            # #     .pow(2)
            # #     .sum(dim=1)
            # # ).mean()
            ...
            # ).mean(dim=1)
            # avg_displacement_loss = (
            #     (cur_deformed_src_pcs - (avg_displacement + nn_init_positions))
            #     .pow(2)
            #     .sum(dim=1)
            # ).mean()
            # total_reg_loss += avg_displacement_loss * ldas_loss_w
        ...
        if i % 20 == 0:
            with torch.no_grad():
                ...
                if use_keypoints:
                    pc_inp = key_points[sample_steps]
                    vis_base_displacement = vis_ori_kp_deforms
                else:
                        ...
                        scale, quaternion = transform_net(
                            torch.cat(
                                [
                                    ...
                                    .to(device),
                                    pred_normals[sample_steps].to(device),
                                ],
                                dim=-1,
                            )
                        )
                ...
                rotation_matrix = quaternion_to_matrix(quaternion)
                deformed_src_pcs = apply_transformation(
                    vis_base_displacement.to(device), scale, rotation_matrix, no_scale=no_scale
                ) + pc_inp.to(device)
                print(f"Loss: {loss.item()}, Reg Loss: {total_reg_loss.item()}, CD Loss: {total_cd_loss.item()}")
                if use_keypoints:
                    deformed_pcs = []
                    for deformed_kp in deformed_src_pcs:
                        deformed_pcs.append(
                            torch.einsum(
                                "ij,ijk->ik",
                                deformed_pc_weights,
                                deformed_kp[deformed_nn_inds],
                            )
                        )
                    deformed_src_pcs = torch.stack(deformed_pcs, dim=0)
                pc_img = plot_pointcloud(deformed_src_pcs, cur_log_dir, title=f"Deformed Point Cloud Iter {i}")
                pc_images.append(pc_img)
        if i % 100 == 0:
            torch.save(
