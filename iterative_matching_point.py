...
def modified_icp_nn_new(
    ...
    for it in tqdm.tqdm(range(iterations), desc="ICP Iterations"):
        ...
        for i in range(len(src)):
            ...
            #     else:
            #         cur_rotated = torch.matmul(R[j], src[i][j])
            #         if torch.sum(rotated[j] - cur_rotated) != 0:
            #             diff_count += 1
            #             # print(f"!!!! diff in part {i} point {j}!!!")
            #         cur_src_transformed.append(cur_rotated + translation[j])
            #         # root_R = R.mean(dim=0)
            #         # root_t = translation.mean(dim=0)
            # cur_src_transformed = torch.stack(cur_src_transformed, dim=0)
            # if torch.sum(src_transformed[i] - cur_src_transformed) != 0:
            #     print(
            #         f"Caught differences in part {i}!!! total num pts different {diff_count}"
            #     )
            #     # print("R and translation shapes", R.shape, translation.shape)

            #     print("src_transformed[i] shape", src_transformed[i].shape)
            #     print("cur_src_transformed shape", cur_src_transformed.shape)
            #     # print out the average differences
            #     print(torch.mean(src_transformed[i] - cur_src_transformed))
            # assert torch.sum(src_transformed[i] - cur_src_transformed) == 0, "differences between src_transformed and cur_src_transformed are not equal to 0"
...
scale = 10.0
log_dir = "fit_pointcloud_logs"
exp_dir = f"imp_wingL{num_keypoints_left}_wingR{num_keypoints_right}_body{num_keypoints_body}"
...
if TRAIN_ICP_MODEL:
    ...
    for time_step in tqdm.tqdm(range(total_time_steps)):
        ...
        icp_models, src_transformed, match_frame, train_iters = modified_icp_nn_new(
            ...
            # lda_weight=0.025,
            early_stop=True,
            stop_volume_ratio=[src_volume_ratio[time_step] for src_volume_ratio in src_volume_ratios],
