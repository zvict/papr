...
class RBFFactory():
    ...
    def b_spline(X, r, k=1):
        ...
        .. math::
            \\varphi(\\boldsymbol{x}) = 
            \\begin{cases}
            \\frac{1}{2} (1 - \\boldsymbol{x})^2
                \\quad & \\text{if}~0 \\leq \\boldsymbol{x} < 1\\\\
            \\frac{1}{2} (2 - \\boldsymbol{x})^2
                \\quad & \\text{if}~1 \\leq \\boldsymbol{x} < 2\\\\
            0
                \\quad & \\text{otherwise}
            \\end{cases}
        ...
        :rtype: float
        """
        arg = 1 - np.power(X / r, k)
        result = np.where(arg < 0, 0, arg)
        return result
