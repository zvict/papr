...
class SimpleMLP(nn.Module):
    def __init__(self, input_dim, hidden_dim=32, out_dim=3):
        ...
        self.fc3 = nn.Linear(hidden_dim, out_dim)  # 1 for scale, 4 for quaternion
...
def modified_icp(
    ...
    return transformations, src_transformed.detach(), cur_frame, it, tgt_nn, tgt_normal_shuffled
...
def finetune_icp(
    ...
    save_dir = os.path.join(log_dir, f"finetune_K{smooth_knn}_W{arap_w}_epoch{total_epoches}/")
    ...
    arap_loss = ARAPLoss(torch.cat(src_ori, dim=0), smooth_knn=smooth_knn)
    ...
    init_volumes = estimate_volume(src_ori[0].cpu().numpy())

    total_time_steps, total_num_pts = tgt.shape[0], tgt.shape[1]

    body_target = [None for _ in range(total_time_steps)]
    ...
    for epoch in tqdm.tqdm(range(total_epoches), desc="Finetune Iterations"):
        epoch_loss = 0.
        epoch_rigid_loss = 0.
        match_frames = []
        all_deformed_pc = []
        for time_step in range(total_time_steps):
            cur_transformed = src_transformed[time_step]
            inp = torch.cat([cur_transformed, point_masks], dim=-1)
            cur_transformed = cur_transformed + model(inp) # TxNx3

            # NOTE: check for part 0 for now
            if estimate_volume(cur_transformed[:len(src_ori[0])].clone().detach().cpu().numpy()) / init_volumes < stop_volume_ratio:
                if body_target[time_step] is None:
                    body_target[time_step] = cur_transformed[:len(src_ori[0])].clone().detach()
                loss = torch.norm(cur_transformed - torch.cat([body_target[time_step], tgt[time_step][len(src_ori[0]):]], dim=0), dim=-1).mean()
            else:
                # l2 loss
                loss = torch.norm(cur_transformed - tgt[time_step], dim=-1).mean()

            # arap loss
            rigid_loss = 0.
            # for i in range(total_time_steps):
            rigid_loss += arap_loss(cur_transformed, add_lda=add_lda, lda_weight=lda_weight)
            loss += rigid_loss * arap_w
            ...
            epoch_rigid_loss += rigid_loss

            all_deformed_pc.append(cur_transformed.clone().detach().unsqueeze(0))

            # if (epoch % 10 == 0 or epoch == total_epoches - 1) and time_step % skip_frame == 0:
            #     cur_frame = plot_pointcloud(
            #         cur_transformed, None, title=f"Match Epoch {epoch} Frame {time_step}",
            #         extra_points=tgt[time_step], plot_match=True
            #     )
            #     match_frames.append(cur_frame)
        all_deformed_pc = torch.cat(all_deformed_pc, dim=0)

        epoch_loss = epoch_loss / total_time_steps
        epoch_rigid_loss = epoch_rigid_loss / total_time_steps
        
        tqdm.tqdm.write(f"Epoch {epoch} Loss: {epoch_loss:.6f}, ARAP Loss: {epoch_rigid_loss:.6f}")

        loss_history.append(epoch_loss)
        # check loss plateau
        if epoch > min_iter and len(loss_history) > min_iter:
            if np.mean(loss_history[-10:]) > np.mean(loss_history[-20:-10]):
                stop = True
            else:
                stop = False
        
        if epoch % 10 == 0 or epoch == total_epoches - 1 or stop:
            match_frames = []
            skip_frame = 10
            for frame_idx in range(0, total_time_steps, skip_frame):
                # plot a vector that connects each point to the corresponding nearest neighbor
                cur_frame = plot_pointcloud(
                    all_deformed_pc[frame_idx], None, title=f"Match Epoch {epoch} Frame {frame_idx}",
                    extra_points=tgt[frame_idx], plot_match=True
                )
                match_frames.append(cur_frame)
            # if len(match_frames):
            imageio.mimsave(os.path.join(save_dir, "match_frames.mp4"), match_frames, fps=10)
            if epoch % 50 == 0:
                torch.save(model.state_dict(), os.path.join(save_dir, f"finetune_model_{epoch}.pth"))
            torch.save(all_deformed_pc, os.path.join(log_dir, f"transferred_tgt_kps_finetuned.pth"))
        if stop:
            break
    torch.save(model.state_dict(), os.path.join(save_dir, f"finetune_model_final.pth"))

    return all_deformed_pc, model
...
if TRAIN_ICP_MODEL:
    ...
    for time_step in tqdm.tqdm(range(total_time_steps)):
        ...
        transformations, src_transformed, match_frame, train_iters, src_tgt, src_normal = modified_icp(
            # [tgt_pc[bird_body_indices], tgt_pc[bird_wing_indices]],
            # [src_pc[but_body_indices], src_pc[but_wing_indices]],
            tgt_kps,
            cur_src_kps,
        ...
        training_src_tgts.append(src_tgt.unsqueeze(0))
        training_src_normals.append(src_normal.unsqueeze(0))
        if time_step == 0:
            print("training_src_tgts shape:", training_src_tgts[0].shape)
            print("training_src_normals shape:", training_src_normals[0].shape)
        torch.save(torch.cat(training_src_tgts, dim=0), os.path.join(log_dir, f"training_src_tgts.pth"))
        torch.save(torch.cat(training_src_normals, dim=0), os.path.join(log_dir, f"training_src_normals.pth"))
    ...
    training_src_tgts = torch.cat(training_src_tgts, dim=0)
    training_src_normals = torch.cat(training_src_normals, dim=0)
    ...
    torch.save(training_src_tgts, os.path.join(log_dir, f"training_src_tgts.pth"))
    torch.save(training_src_normals, os.path.join(log_dir, f"training_src_normals.pth"))
else:
    transferred_tgt_kps = torch.load(os.path.join(log_dir, f"transferred_tgt_kps.pth"))
    training_src_tgts = torch.load(os.path.join(log_dir, f"training_src_tgts.pth"))
    training_src_normals = torch.load(os.path.join(log_dir, f"training_src_normals.pth"))
    print("Loaded", transferred_tgt_kps.shape)

# exit(0)

FINETUNE_TRANSFORMATION = True
# FINETUNE_TRANSFORMATION = False

if FINETUNE_TRANSFORMATION:
    model = SimpleMLP(4, hidden_dim=32, out_dim=3).to(device)
    # point_masks = torch.cat([
    #     torch.ones(1, len(tgt_kp), 1, dtype=torch.float32, device=device) * part_idx for part_idx, tgt_kp in enumerate(tgt_kps)
    # ], dim=1)
    point_masks = torch.cat([
        torch.ones(len(tgt_kp), 1, dtype=torch.float32, device=device) * part_idx for part_idx, tgt_kp in enumerate(tgt_kps)
    ], dim=0)
    transferred_tgt_kps, model = finetune_icp(
            tgt_kps,
            transferred_tgt_kps,
            training_src_tgts,
            training_src_normals,
    ...
    torch.save(transferred_tgt_kps, os.path.join(log_dir, f"transferred_tgt_kps_finetuned.pth"))

else:
    transferred_tgt_kps = torch.load(os.path.join(log_dir, f"transferred_tgt_kps_finetuned.pth"))
