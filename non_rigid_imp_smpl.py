...
def modified_icp_nn_new(
    ...
    if use_arap and not joint_arap:
        arap_losses = {name: ARAPLoss(deforming_points.nodes_by_name[name].points, smooth_knn=smooth_knn) for name in reference_points.keys()}
    ...
    total_num_pts = sum([len(c_src) for c_src in reference_points.values()])

    if correspondence == "linear_sum" and reference_normals is None:
        reference_normals = {
            name: estimate_normals_pytorch3d(reference_points[name].cpu().numpy())
            for name in reference_points.keys()
        }
    ...
    for it in tqdm.tqdm(range(iterations), desc="IMP Iterations"):
        ...
        input_pts, masks = deforming_points.generate_model_input()
        params = model(input_pts, masks)  # (N, 6)
        deforming_points.store_model_output(params)
        deforming_points.update_world_transforms()

        for name in sorted(reference_points.keys()):
            ...
            reference_kps = reference_points[name]
...
ref_obj_mesh = trimesh.load(
    "/NAS/spa176/skeleton-free-pose-transfer/demo/src_data_smpl/1.obj", process=False
...
for name, indices in part_indices.items():
    if USE_KEYPOINTS:
        # FPS
        cur_src_kp, src_kp_idx = sfp(
            src_pc[indices].unsqueeze(0),
            K=all_num_keypoints[name],
        )
...
if USE_KEYPOINTS:
    ...
    for cur_step in tqdm.tqdm(range(total_time_steps)):
        ...
        for name in sorted(src_kps.keys()):
            ...
            total_deformed_pcs[part_indices[name]] = deformed_full_point_cloud
