...
def _partial_overlap_metrics(src_aligned: torch.Tensor,
                             tgt: torch.Tensor,
                             tau: float = None):
    """
    Compute robust alignment metrics for possibly partial overlap.
    ...
    with torch.no_grad():
        ...
        d_st_full = torch.cdist(src_aligned, tgt)              # (Ns, Nt)
        d_st, _ = d_st_full.min(dim=1)                         # source -> target
        d_ts, _ = d_st_full.min(dim=0)                         # target -> source (reuse matrix)

        median = d_st.median()
        p90 = torch.quantile(d_st, 0.90)

        # Adaptive threshold if not supplied
        if tau is None:
            # Guard tiny median
            base = median.clamp(min=1e-6)
            tau = 2.5 * base

        precision = (d_st < tau).float().mean()
        recall    = (d_ts < tau).float().mean()
        fscore = (2 * precision * recall) / (precision + recall + 1e-8)

        # Avoid selecting degenerate collapses: require minimal spread
        src_spread = src_aligned.std(dim=0).mean()
        deg_penalty = 0.5 if src_spread < 1e-3 else 0.0

        # Composite score (lower better)
        score = median + 0.5 * p90 - 0.6 * fscore + deg_penalty
...
class DeformationModel(nn.Module):
    ...
    def forward(self):
        ...
        if self.weight_logits.requires_grad:
            ...
            # Reconstruct one-hot from original labels if weights frozen
        ...
        if any(p.requires_grad for p in self.mlp.parameters()):
            mlp_input = torch.cat([self.source_vertices, weights], dim=1)
            displacements = self.mlp(mlp_input)
            return skinned_vertices + displacements, skinned_vertices, displacements
...
def run_pipeline(
    ...
    run_stage(
        ...
        matching_interval=(10, 100),   # start every 10 steps, relax to every 100
