...
if TRAIN_ICP_MODEL:
    ...
    for time_step in tqdm.tqdm(range(total_time_steps)):
        ...
        if time_step == 0:
            for part_idx in range(total_part_num):
                cur_src_kp_np = cur_src_kps[part_idx].cpu().numpy()
                cur_src_normal = estimate_normals_pytorch3d(cur_src_kp_np) 
                _, _, tgt_indices = linear_sum_correspondence(
                    tgt_kps[part_idx].cpu().numpy(), cur_src_kp_np, cur_src_normal, use_cos=False, alpha=0.75
                )
                initial_tgt_indices.append(tgt_indices)
