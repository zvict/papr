...
def test_step(frame, num_frames, model, device, dataset, batch, loss_fn, lpips_loss_fn_alex, lpips_loss_fn_vgg, args, test_losses, test_psnrs, test_ssims, test_lpips_alexs, test_lpips_vggs, resume_step, resample, shading_codes, seed, i):
    ...
    if resample == False:
        ...
        resample_shading_codes(shading_codes, args, model, dataset, idx, loss_fn, resume_step)
    ...
    if args.test.save_fig:
        ...
        log_dir = os.path.join(args.save_dir, args.index, 'test', f'exposure_control_test_resample{resample}_seed{seed}')
...
def test(model, device, dataset, save_name, args, resume_step, resample, shading_codes, test_frame, seed):
    ...
    for frame, batch in enumerate(testloader):
        ...
        for i in range(20):
            ...
            plots = test_step(frame, len(testloader), model, device, dataset, batch, loss_fn, lpips_loss_fn_alex,
                            ...
                            test_lpips_vggs, resume_step, resample, shading_codes, seed, i)
...
def main(args, save_name, mode, resume_step=0, resample=False, frame=0, seed=0):
    ...
    test(model, device, dataset, save_name, args, resume_step, resample, shading_codes, frame, seed)
...
if __name__ == '__main__':
    ...
    for i, dataset in enumerate(config['test']['datasets']):
        ...
        main(args, name, mode, resume_step, resample, frame, seed)
