...
def plot_pointcloud(points, save_dir, title="", extra_points=None):
    ...
    for i in range(num_pc):
        ...
        if extra_points is not None:
            ...
            colors = x_e
            ax.scatter3D(x_e, y_e, z_e, c=colors, cmap='magma', alpha=0.5)
...
exp_dir = f"ot_wingL{num_keypoints_left}_wingR{num_keypoints_right}_body{num_keypoints_body}"
if icp_instance:
    exp_dir += "_icp"
if USE_COS:
    exp_dir += "_cos"
...
if REG_DEFORM_NET:
    ...
    def save_deformed_kps(shift=True):
        with torch.no_grad():
            for part_idx in range(total_part_num):
                if use_keypoints:
                    if input_case == 2 or input_case == 3:
